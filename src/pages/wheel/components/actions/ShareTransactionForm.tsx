...
export const ShareTransactionForm: React.FC<ShareTransactionFormProps> = ({
  type,
  prefillData,
  onClose,
}) => {
  ...
  const [shares, setShares] = useState(prefillData?.shares || 0);
  const [pricePerShare, setPricePerShare] = useState(prefillData?.pricePerShare || 0);
  ...
  const handleSubmit = async () => {
    if (!symbol || shares <= 0 || pricePerShare <= 0) {
    ...
    try {
      ...
      const template = type === 'buy' ? tmplBuyShares : tmplSellShares;
      ...
      showSuccess(
        `✅ Share ${type === 'buy' ? 'purchase' : 'sale'} recorded: ${shares} ${symbol.toUpperCase()} shares at $${pricePerShare.toFixed(2)}`
      );

      onClose();
    } catch (error) {
      console.error('Failed to record transaction:', error);
      showError('Failed to record transaction. Check console for details.');
    } finally {
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="modal w-full max-w-md">
        <div className="modal-header">
          <h2 className="modal-title">{type === 'buy' ? 'Buy Shares' : 'Sell Shares'}</h2>
        ...
        <div className="space-y-4 p-6">
          ...
          <div className="rounded-lg border border-[rgba(245,179,66,0.3)] bg-[rgba(11,15,14,0.6)] p-4">
            ...
            <div className="space-y-1 text-sm text-slate-300">
              <div>
                {shares} share{shares !== 1 ? 's' : ''} × ${pricePerShare.toFixed(2)}
              </div>
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              fullWidth
              disabled={isSubmitting || !symbol || shares <= 0 || pricePerShare <= 0}
            >
              {isSubmitting ? 'Recording...' : type === 'buy' ? 'Record Purchase' : 'Record Sale'}
            </Button>
          </div>
        </div>
      </div>
