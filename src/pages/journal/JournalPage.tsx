...
const JournalPage: React.FC = () => {
  ...
  React.useEffect(() => {
    if (symbol && symbol.length > 0) {
      const filtered = uniqueSymbols.filter(s => 
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(() => {
    ...
    if (saved) {
      try {
        ...
        return new Set(['date', 'symbol', 'type', 'qty', 'strike', 'exp', 'dte', 'stock', 'amount', 'notes', 'delta', 'iv_rank', 'actions']);
      }
    }
    return new Set(['date', 'symbol', 'type', 'qty', 'strike', 'exp', 'dte', 'stock', 'amount', 'notes', 'delta', 'iv_rank', 'actions']);
  });
  ...
  // Grouped view state
  const [groupBy, setGroupBy] = useState<'none' | 'symbol' | 'date'>(() => {
  ...
  const groupedEntries = React.useMemo(() => {
    ...
    return Array.from(groups.entries()).map(([key, entries]) => {
      const sorted = sortField ? [...entries].sort((a, b) => {
        let aVal: any;
        let bVal: any;
        ...
        switch (sortField) {
          ...
          case 'strike':
            ...
            break;
          case 'dte':
            const aDte = a.expiration ? Math.ceil((new Date(a.expiration).getTime() - new Date(a.ts).getTime()) / 86400000) : null;
            const bDte = b.expiration ? Math.ceil((new Date(b.expiration).getTime() - new Date(b.ts).getTime()) / 86400000) : null;
            aVal = aDte ?? 9999;
            bVal = bDte ?? 9999;
            break;
          default:
  ...
  const entriesToPaginate = groupBy === 'none' ? sortedEntries : (groupedEntries?.flatMap(g => g.entries) ?? []);
  ...
  // Toast notifications
  const [toasts, setToasts] = useState<Array<{ id: string; message: string; type?: 'success' | 'error' | 'info' | 'warning'; undo?: () => void }>>([]);
  
  const showToast = (message: string, type?: 'success' | 'error' | 'info' | 'warning', undo?: () => void) => {
    ...
    try {
      ...
      setTimeout(() => {
        setToasts(prev => prev.map(toast => 
          toast.undo === undoHandler ? { ...toast, undo: undefined } : toast
        ));
      }, 5000);
    } catch (err) {
  ...
  const validateForm = (): boolean => {
    ...
    if ((tmpl === 'sellPut' || tmpl === 'sellCC' || tmpl === 'putAssigned' || tmpl === 'callAssigned')) {
      if (contracts <= 0) {
    ...
    <div className="cyber-bg relative min-h-screen overflow-hidden bg-black text-zinc-200">
      ...
      <header className="page-header">
        <div className="page-header__inner">
          <h1 className="page-header__brand page-header__brand--cyberpunk" data-testid="journal.title">💼 Wheel Strategy Journal</h1>
          ...
          <div className="flex gap-2">
            <div className="flex items-center gap-2">
              <button
                ...
                className="page-header__btn page-header__btn--secondary no-print"
                title={viewMode === 'compact' ? 'Switch to detailed view' : 'Switch to compact view'}
                aria-label={viewMode === 'compact' ? 'Switch to detailed view' : 'Switch to compact view'}
              >
                {viewMode === 'compact' ? '📋 Detailed' : '📊 Compact'}
              </button>
              <button
                onClick={() => {
                  const options = ['none', 'symbol', 'date'];
                  const currentIndex = options.indexOf(groupBy);
                  const nextIndex = (currentIndex + 1) % options.length;
                  setGroupBy(options[nextIndex] as 'none' | 'symbol' | 'date');
                }}
                className="page-header__btn page-header__btn--secondary no-print"
                title="Group by symbol or date"
                aria-label="Toggle grouping"
              >
                {groupBy === 'none' ? '📊 Group' : groupBy === 'symbol' ? '📊 By Symbol' : '📅 By Date'}
              </button>
              <button
                onClick={() => {
        ...
        <div
          ...
          className="absolute right-4 top-20 z-50 hidden rounded-lg border border-zinc-700 bg-zinc-900 p-4 shadow-lg no-print"
          style={{ display: 'none' }}
        >
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 flex items-center justify-around border-t border-zinc-800 bg-zinc-900/95 py-3 backdrop-blur-sm md:hidden no-print">
        <button
          onClick={() => setOpen(true)}
          className="flex flex-col items-center gap-1 rounded-lg px-4 py-2 text-green-400 transition-colors active:bg-zinc-800 touch-manipulation"
        ...
        <button
          ...
          className="flex flex-col items-center gap-1 rounded-lg px-4 py-2 text-zinc-400 transition-colors active:bg-zinc-800 touch-manipulation"
          aria-label="Columns"
        >
        ...
        <button
          onClick={() => setViewMode(viewMode === 'compact' ? 'detailed' : 'compact')}
          className="flex flex-col items-center gap-1 rounded-lg px-4 py-2 text-zinc-400 transition-colors active:bg-zinc-800 touch-manipulation"
          aria-label={viewMode === 'compact' ? 'Detailed view' : 'Compact view'}
        >
          <span className="text-xl">{viewMode === 'compact' ? '📋' : '📊'}</span>
          <span className="text-xs">{viewMode === 'compact' ? 'Detailed' : 'Compact'}</span>
        </button>
        <button
          onClick={() => {
            const options = ['none', 'symbol', 'date'];
          ...
          aria-label="Group"
        >
          <span className="text-xl">{groupBy === 'none' ? '📊' : groupBy === 'symbol' ? '📊' : '📅'}</span>
      ...
      {/* Main Content */}
      <div className="flex justify-center px-4 py-8 pb-20 sm:px-6 lg:px-8 md:pb-8">
        <main className="relative w-full max-w-7xl">
          {/* Summary Cards */}
          <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3 no-print">
            <div className="rounded-2xl border border-green-500/30 bg-linear-to-br from-black/80 to-green-950/20 p-4 shadow-lg shadow-green-500/20 backdrop-blur-xl">
              <div className="text-xs uppercase tracking-wide text-zinc-500">Total Premium</div>
              <div className="mt-1 text-2xl font-bold text-green-400">{fmtMoney(wheelCalcs.totalIn)}</div>
            </div>
            <div className="rounded-2xl border border-green-500/30 bg-linear-to-br from-black/80 to-green-950/20 p-4 shadow-lg shadow-green-500/20 backdrop-blur-xl">
              <div className="text-xs uppercase tracking-wide text-zinc-500">Net P/L</div>
              <div className={`mt-1 text-2xl font-bold ${wheelCalcs.netPL >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {fmtMoney(wheelCalcs.netPL)}
              </div>
            </div>
            <div className="rounded-2xl border border-green-500/30 bg-linear-to-br from-black/80 to-green-950/20 p-4 shadow-lg shadow-green-500/20 backdrop-blur-xl">
              <div className="text-xs uppercase tracking-wide text-zinc-500">Active Positions</div>
              <div className="mt-1 text-2xl font-bold text-zinc-300">{wheelCalcs.byTicker.length}</div>
            </div>
          </div>
          ...
          {showCharts && (premiumChartData.length > 0 || pnlChartData.length > 0) && (
            <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 no-print">
          ...
          {wheelCalcs.byTicker.length > 0 && (
            <div className="neon-panel mb-6 rounded-2xl px-6 py-4 no-print">
              ...
              <>
                ...
                <div className="block md:hidden space-y-4">
                  {paginatedEntries.map((r: Entry) => {
                    const dte = r.expiration
                      ? Math.ceil((new Date(r.expiration).getTime() - new Date(r.ts).getTime()) / 86400000)
                      : null;
                    return (
                      <div
                        key={r.id}
                        className="rounded-xl border border-zinc-700/60 bg-zinc-900/60 p-4 shadow-lg"
                      >
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          ...
                          <div className="col-span-2">
                            ...
                            <span className={`ml-2 font-semibold ${r.amount >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                              {fmtMoney(r.amount)}
                            </span>
                          </div>
                        </div>
                  ...
                  {entriesToPaginate.length === 0 && (
                    <div className="rounded-xl border border-zinc-700/60 bg-zinc-900/60 p-4 text-center text-zinc-500">
                      {filters.symbol || filters.type || filters.from || filters.to ? (
                        <div className="space-y-2">
                          <p className="font-semibold text-zinc-400">No entries match your filters</p>
                          <button
                ...
                <div className="hidden md:block">
                  ...
                  <thead>
                    <tr>
                          <th 
                        className="text-left cursor-pointer select-none hover:text-zinc-200 transition-colors"
                      ...
                      <th className={`text-left ${visibleColumns.has('type') ? '' : 'hidden'}`} aria-label="Trade type">Type</th>
                      <th className={`text-right ${visibleColumns.has('qty') ? '' : 'hidden'}`} aria-label="Quantity">Qty</th>
                      <th 
                        className={`text-right cursor-pointer select-none hover:text-zinc-200 transition-colors ${visibleColumns.has('strike') ? '' : 'hidden'}`}
                      ...
                      <th className={`${visibleColumns.has('exp') ? '' : 'hidden'}`} aria-label="Expiration date">Exp</th>
                      <th 
                        className={`text-right cursor-pointer select-none hover:text-zinc-200 transition-colors ${visibleColumns.has('dte') ? '' : 'hidden'}`}
                      ...
                      <th className={`text-right ${visibleColumns.has('stock') ? '' : 'hidden'}`} aria-label="Stock price">Stock</th>
                      <th 
                        className={`text-right cursor-pointer select-none hover:text-zinc-200 transition-colors ${visibleColumns.has('amount') ? '' : 'hidden'}`}
                      ...
                      <th className={`text-left ${visibleColumns.has('notes') ? '' : 'hidden'}`} aria-label="Notes">Notes</th>
                      <th className={`text-right ${visibleColumns.has('delta') ? '' : 'hidden'}`} aria-label="Delta">Delta</th>
                      <th className={`text-right ${visibleColumns.has('iv_rank') ? '' : 'hidden'}`} aria-label="IV Rank">IV Rank</th>
                      <th className={`text-center ${visibleColumns.has('actions') ? '' : 'hidden'}`} aria-label="Actions">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedEntries.map((r: Entry) => {
                      ...
                      return (
                        ...
                        <tr 
                          ...
                          className="transition-all duration-200 hover:bg-zinc-800/40 cursor-pointer touch-manipulation"
                          onClick={() => {
                          ...
                          >
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1' : 'py-2'} ${visibleColumns.has('date') ? '' : 'hidden'}`}>
                            {viewMode === 'detailed' && (
                              <button
                          ...
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} font-semibold ${visibleColumns.has('symbol') ? '' : 'hidden'}`}>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                          ...
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1' : 'py-2'} ${visibleColumns.has('type') ? '' : 'hidden'}`}>
                            <span
                              className={`inline-block rounded px-2 py-0.5 text-xs ${
                                r.type === 'sell_to_open'
                                  ? 'bg-green-500/20 text-green-400'
                          ...
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} text-right text-zinc-300 ${visibleColumns.has('qty') ? '' : 'hidden'}`}>{r.qty ?? '—'}</td>
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} text-right ${visibleColumns.has('strike') ? '' : 'hidden'}`}>
                            {r.strike ? `$${r.strike.toFixed(2)}` : '—'}
                          </td>
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} text-zinc-400 ${visibleColumns.has('exp') ? '' : 'hidden'}`}>
                            {r.expiration ? fmtDate(r.expiration) : '—'}
                          </td>
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} text-right font-semibold ${visibleColumns.has('dte') ? '' : 'hidden'} ${
                            dte !== null && dte >= 0
                              ? dte < 7
                                ? 'text-red-400'
                                : dte < 30
                          ...
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} text-right text-zinc-400 ${visibleColumns.has('stock') ? '' : 'hidden'}`}>
                            {r.underlying_price ? `$${r.underlying_price.toFixed(2)}` : '—'}
                          </td>
                          <td
                            className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} text-right font-semibold ${visibleColumns.has('amount') ? '' : 'hidden'} ${r.amount >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
                          >
                            {fmtMoney(r.amount)}
                          </td>
                          <td className={`max-w-[150px] px-3 ${viewMode === 'compact' ? 'py-1 text-xs' : 'py-2 text-xs'} text-zinc-500 ${visibleColumns.has('notes') ? '' : 'hidden'}`}>
                            {r.notes ? (
                              <span
                                className={`cursor-pointer block ${expandedNotes.has(r.id) ? 'whitespace-normal' : 'truncate'}`}
                                title={expandedNotes.has(r.id) ? 'Click to collapse' : r.notes}
                                onClick={(e) => {
                                  e.stopPropagation();
                          ...
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} text-right text-zinc-400 ${visibleColumns.has('delta') ? '' : 'hidden'}`}>
                            {r.meta && typeof r.meta === 'object' && 'delta' in r.meta
                              ? (typeof r.meta.delta === 'number' ? r.meta.delta.toFixed(2) : '—')
                              : '—'}
                          </td>
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1 text-sm' : 'py-2'} text-right text-zinc-400 ${visibleColumns.has('iv_rank') ? '' : 'hidden'}`}>
                            {r.meta && typeof r.meta === 'object' && 'iv_rank' in r.meta
                              ? (typeof r.meta.iv_rank === 'number' ? r.meta.iv_rank : '—')
                              : '—'}
                          </td>
                          <td className={`px-3 ${viewMode === 'compact' ? 'py-1' : 'py-2'} text-center ${visibleColumns.has('actions') ? '' : 'hidden'}`}>
                            <button
                              onClick={() => handleEditClick(r)}
                              className="mr-2 min-w-[44px] min-h-[44px] text-blue-400 transition-colors hover:text-blue-300 touch-manipulation"
                              title="Edit entry"
                            ...
                            <button
                              onClick={() => handleDeleteClick(r)}
                              className="min-w-[44px] min-h-[44px] text-red-400 transition-colors hover:text-red-300 touch-manipulation"
                              title="Delete entry"
                            ...
                            <td colSpan={13} className="px-6 py-4">
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                  ...
                                  <span className="ml-2 text-zinc-300">{r.account_id || '—'}</span>
                                </div>
                                {r.meta && typeof r.meta === 'object' && (
                                  <>
                                    ...
                                    {'iv_percentile' in r.meta && (
                                      <div>
                                        <span className="text-zinc-500">IV Percentile:</span>
                                        <span className="ml-2 text-zinc-300">
                                          {typeof r.meta.iv_percentile === 'number' ? r.meta.iv_percentile : '—'}
                                        </span>
                                      </div>
                                    )}
                                    {'commission' in r.meta && (
                                      <div>
                                        ...
                                        <span className="ml-2 text-zinc-300">
                                          {typeof r.meta.commission === 'number' ? `$${r.meta.commission.toFixed(2)}` : '—'}
                                        </span>
                                      </div>
                                    )}
                                  </>
                                ...
                                {(() => {
                                  const related = entries.filter(e => 
                                    ...
                                    (r.expiration ? e.expiration === r.expiration : true)
                                  ).slice(0, 5);
                                  
                                  // Check for edit history (original_entry_id)
                                  const edits = entries.filter(e => 
                                    e.original_entry_id === r.id || r.original_entry_id === e.id
                                  );
                                  
                                  if (related.length === 0 && edits.length === 0) return null;
                                  
                                  return (
                                    <div className="col-span-2 space-y-2">
                                      {related.length > 0 && (
                                        <div>
                                          <span className="text-zinc-500">Related Entries ({related.length}):</span>
                                            ...
                                            {related.map((rel) => (
                                              <button
                                                ...
                                                onClick={(e) => {
                                                  ...
                                                  document.getElementById(`entry-${rel.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                                }}
                                                className="rounded border border-blue-500/50 bg-blue-900/20 px-2 py-1 text-xs text-blue-400 hover:bg-blue-900/30 transition-colors touch-manipulation"
                                              >
                                                {fmtDate(rel.ts)} - {rel.type.replace(/_/g, ' ')}
                                              </button>
                                            ))}
                                          </div>
                                        </div>
                                      ...
                                      {edits.length > 0 && (
                                        <div>
                                          <span className="text-zinc-500">Edit History ({edits.length}):</span>
                                          <div className="mt-2 flex flex-wrap gap-2">
                                            {edits.map((edit) => (
                                              <button
                                                key={edit.id}
                                                onClick={(e) => {
                                                  ...
                                                  document.getElementById(`entry-${edit.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                                }}
                                                className="rounded border border-amber-500/50 bg-amber-900/20 px-2 py-1 text-xs text-amber-400 hover:bg-amber-900/30 transition-colors touch-manipulation"
                                              >
                                                {fmtDate(edit.ts)} - {edit.edit_reason || 'Edited'}
                                              </button>
                                            ))}
                                          </div>
                                        </div>
                    ...
                    {entriesToPaginate.length === 0 && (
                      <tr>
                        <td className="px-3 py-12 text-center text-zinc-500" colSpan={13}>
                          {filters.symbol || filters.type || filters.from || filters.to ? (
                            <div className="space-y-2">
                              <p className="text-lg font-semibold text-zinc-400">No entries match your filters</p>
                              <p className="text-sm">Try adjusting your filters or clearing them to see all entries.</p>
                              <button
                                onClick={() => filters.resetFilters()}
                            ...
                            <div className="space-y-2">
                              <p className="text-lg font-semibold text-zinc-400">No entries yet</p>
                              <p className="text-sm">Start tracking your wheel strategy by creating your first entry.</p>
                              <button
                                onClick={() => setOpen(true)}
                                className="mt-2 rounded-lg border border-green-500/50 bg-green-500/20 px-4 py-2 text-sm text-green-400 transition-colors hover:bg-green-500/30"
                              >
                    ...
                    <div className="space-y-6">
                      {(paginatedGroups ?? groupedEntries ?? []).map(({ key, entries, total, count }) => (
                        <div key={key} className="rounded-xl border border-zinc-700/60 bg-zinc-900/60">
                              ...
                              <tbody>
                                {entries.map((r: Entry) => {
                                  const dte = r.expiration
                                    ...
                                    : null;
                                  return (
                                    <tr key={r.id} className="transition-all duration-200 hover:bg-zinc-800/40 cursor-pointer">
                                      ...
                                      <td className="px-3 py-2 text-right">{r.strike ? `$${r.strike.toFixed(2)}` : '—'}</td>
                                      ...
                                      <td className={`px-3 py-2 text-right font-semibold ${r.amount >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                        {fmtMoney(r.amount)}
                                      </td>
                ...
                {/* Pagination Controls */}
                {entriesToPaginate.length > 0 && (
                  <div className="mt-4 flex items-center justify-between border-t border-zinc-700/60 px-4 py-3 no-print">
                    <div className="flex items-center gap-4">
                      <span className="text-sm text-zinc-400">
                        Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, entriesToPaginate.length)} of {entriesToPaginate.length} entries
                      </span>
                      <select
                        value={pageSize}
                        onChange={(e) => {
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="rounded border border-zinc-700 bg-zinc-900 px-3 py-1 text-sm text-zinc-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-zinc-800"
                      >
                        First
                      </button>
                      <button
                ...
                {selectedEntries.size > 0 && (
                  <div className="mt-4 flex items-center justify-between rounded-lg border border-green-500/30 bg-green-900/20 px-4 py-3 no-print">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={async () => {
                          const confirmed = window.confirm(`Delete ${selectedEntries.size} entries?`);
                          if (!confirmed) return;
                          ...
                          for (const id of selectedEntries) {
                            try {
                              ...
                              showToast(`Failed to delete entry: ${err instanceof Error ? err.message : String(err)}`, 'error');
                            }
                          }
                          setSelectedEntries(new Set());
                          showToast(`${selectedEntries.size} entries deleted successfully`, 'success');
                        }}
                        className="rounded border border-red-500/50 bg-red-900/20 px-3 py-1 text-sm text-red-400 hover:bg-red-900/30"
                      >
                        Delete Selected
                      ...
                      <button
                        onClick={() => {
                          ...
                          const url = URL.createObjectURL(blob);
                          link.setAttribute('href', url);
                          link.setAttribute('download', `journal-selected-${new Date().toISOString().slice(0, 10)}.csv`);
                          link.style.visibility = 'hidden';
                          document.body.appendChild(link);
                          link.click();
                          document.body.removeChild(link);
      ...
      <Modal isOpen={open} onClose={() => {
        setOpen(false);
        setErrors({});
      }} title="✨ New Entry" size="xl">
        <div className="space-y-4">
          {/* Template Selector */}
          <div className="mb-4">
            <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-400">Template Type</div>
          ...
          {(tmpl === 'sellPut' || tmpl === 'sellCC') && (
            <div className="mb-4">
              <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-400">Quick Presets</div>
              <div className="flex gap-2">
                  <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => {
                    ...
                    if (strike > 0) {
                      const estimatedPremium = strike * 0.30 * 0.01;
            ...
            <>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {errors.contracts && <p className="mt-1 text-xs text-red-400">{errors.contracts}</p>}
                </div>
                <div>
                  <Input
                    label="Strike"
              ...
              {(tmpl === 'sellPut' || tmpl === 'sellCC') && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    />
                    {errors.premium && <p className="mt-1 text-xs text-red-400">{errors.premium}</p>}
                  </div>
                  <div>
                    <Input
